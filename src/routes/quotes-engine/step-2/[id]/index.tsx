...
export const head: DocumentHead = {
    ...
    meta : [{
        name: 'Paso 2',
        content: 'Tus datos y complementos',
    }]
}
...
export default component$(() => {
    ...
    useVisibleTask$(async() => {
        ...
        await ServiceRequest('/pw_getCotizacion',{idcotizacion:Number(location.params.id.split('-')[0])},(response) => {
            newResume.idagencia = response.resultado[0].idagencia
            newResume.idcotizacion = response.resultado[0].idcotizacion
            newResume.idusuario = response.resultado[0].idusuario
            newResume.desde = JSON.parse(response.resultado[0].jsoncotizacion).cotizacion.fecha.desde
            newResume.hasta = JSON.parse(response.resultado[0].jsoncotizacion).cotizacion.fecha.hasta
            newResume.dias = Number(JSON.parse(response.resultado[0].jsoncotizacion).cotizacion.fecha.dias)
            newResume.origen = JSON.parse(response.resultado[0].jsoncotizacion).cotizacion.origen
            newResume.destinos = JSON.parse(response.resultado[0].jsoncotizacion).cotizacion.destinos
            ...
            destinations.value.map(destination => {
                newResume.destinos.map((destino:any,index:number) => {
                    if(destination.value == destino)
                    {
                        if(index == (newResume.destinos.length - 1))
                        {
            ...
            JSON.parse(response.resultado[0].jsoncotizacion).cotizacion.pasajeros.map((pax:any) => {
                if(pax.edad <= 22)
                {
                    newResume[22] += 1
            ...
            plans.value = response.resultado[0].planescotizados
            
            response.resultado[0].planescotizados.map((plancotizado:any) => {
        ...
        if(Object.keys(stateContext.value).length > 0)
        {
            ...
            if(prevResume.asegurados != undefined)
            {
                ...
                totalPay.value = {divisa:resume.value.plan.codigomonedapago,total:resume.value.plan.precio_grupal}
            ...
            else
            {
                ...
                // await ServiceRequest('https://servicios-desarrollo-backend.continentalassist.com/pw_getCotizadorViajesBeneficiosAdicionales',dataRequest,(response) => {
                //         newRes = response.resultado
                //     })
        ...
        if(checkPolicy.checked == true)
        {
            forms.map((form,index) => {
                if(!form.checkValidity())
                {
                    ...
                    const card : any = document.querySelector('#card-'+(index+1)) as HTMLElement
            ...
            if(!error.includes(true))
            {
                ...
                // await ServiceRequest('https://servicios-desarrollo-backend.continentalassist.com/pw_getCotizadorValidaPasajeros',dataRequest,(response) => {
                //     resValidPaxs = response
                // })
                    ...
                    if(paxs.length > 0)
                    {
                        ...
                        if(paxs.length == countPaxs)
                        {
                            ...
                            (window as any)['dataLayer'].push({
                                'event': 'TrackEvent',
                                'Category': 'Cotizador',
                                'Action': 'Paso 3 :: Resumen de compra',
                                ...
                                'Ninosjovenes': newStateContext[22],
                                'Adultosmayores': newStateContext[85],
                                'Value': newStateContext.total.total
                            });
    ...
    const getAdditional$ = $((index:number,idpax:number,benefit:object) => {
        ...
        dataBenefits.map((pax,indexP) => {
            if(pax.idpasajero == idpax)
            {
                ...
                dataBenefits[indexP].beneficiosadicionalesasignados[index].selected = true
                ...
                totalPay.value.total += dataBenefits[indexP].beneficiosadicionalesasignados[index].precio
            }
        })
    })
            ...
            if(pax.idpasajero == idpax)
            {
                dataBenefits[indexP].beneficiosadicionales.map(((item,indexI) => {
                    if(item.idbeneficioadicional == benefit.idbeneficioadicional)
                    {
                        ...
                        dataBenefits[indexP].beneficiosadicionalesasignados[index].selected = false
                            ...
                            <div class='row align-content-center justify-content-center'>
                                {
                                    ...
                                    <div class='col-xl-10 text-center mb-3 mt-5'>
                                        ...
                                        <h2 class='h5 text-dark-blue'>Ingresa la información de los viajeros <br class='mobile'/> y elige beneficios opcionales</h2>
                                    </div>
                                    :
                                    <div class='col-lg-12 text-center mt-5 mb-5'>
                                        <h2 class='h1 text-semi-bold text-dark-blue'>Lo sentimos!</h2>
                                        <h3 class='h5 text-dark-blue'>Hubo un error en la búsqueda, vuelve a intentarlo.</h3>
                                    </div>
                                }
                            </div>
                            <div class='row mb-5'>
                                                                    ...
                                                                    <div class="row align-items-center">
                                                                        <div class="col-xl-12">  
                                                                            <Form
                                                                                ...
                                                                                form={[
                                                                                    {row:[
                                                                                        {size:'col-xl-4',type:'text',label:'Nombre(s)',name:'nombres',required:true,value:addBenefit.nombres},
                                                                                        {size:'col-xl-4',type:'text',label:'Apellido(s)',name:'apellidos',required:true,value:addBenefit.apellidos},
                                                                                    ...
                                                                                    {row:[
                                                                                        ...
                                                                                        {size:'col-xl-4',type:'tel',label:'Teléfono',name:'telefono',required:true,value:addBenefit.telefono},
                                                                                    ]}
                                                                                ]}
                                                                            />   
                                                                    ...
                                                                    <div class='row'>
                                                                        <div class='col-lg-12'>
                                                                            <Form
                                                                                ...
                                                                                form={[
                                                                                    {row:[
                                                                                        {size:'col-xl-8',type:'text',label:'Nombre(s)',name:'nombre_contacto',required:true},
                                                                                        {size:'col-xl-4',type:'tel',label:'Teléfono',name:'telefono_contacto',required:true},
                                                                                    ]},
                                                                                    {row:[
                                                                                        {size:'col-xl-12',type:'email',label:'Correo',name:'email_contacto',required:true},
                                                                                    ]}
                                                                                ]}
                                                                            />   
                                                                        </div>
                                                    ...
                                                    <div class='col text-center'>
                                                        <div class="form-check form-check-inline">
                                                            ...
                                                            <label class="form-check-label" for={"check-policy"}>
                                                                Aceptas nuestra <a href='https://storage.googleapis.com/files-continentalassist-web/continental-assist-poli%CC%81tica-de-tratamiento-de-la-informacio%CC%81n-y-privacidad.pdf' target='_black'>Política de Tratamiento de la Información y Privacidad</a>.
                                                            </label>
                                                        </div>
                                                    </div>
                                                </div>
                                                            ...
                                                            <div class='d-grid gap-2'>
                                                                {
                                                                    ...
                                                                    :
                                                                    <button class='btn btn-outline-primary' onClick$={() => {getAdditional$(index,additionalsBenefitsPlan.value.idpasajero,benefit)}}>Agregar</button>
                                                                }
                                                            </div>
                                                        </div>
                                                    </div>
